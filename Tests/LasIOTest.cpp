...
TEST(IOTEST, readtest)
{
    ...
    // homebrew lasreader
    LasReader lasReader(fileName);

    for (LasReader::const_iterator it = lasReader.cbegin(); it < lasReader.cend(); it++) {
        cout << it->getXCoordinate() << "," << it->getYCoordinate() << "," << it->getZCoordinate() << endl;
    }
